...
team_t team = {
  ...
  "rast4675@colorado.edu",
  /* Second member's full name (leave blank if none) */
  "Bryan Moreno",
  /* Second member's email address (leave blank if none) */
  "brmo3229@colorado.edu"
};
...
static void *find_fit(uint32_t asize)
{
  ...
  for (bp = save_p; GET_SIZE(HDRP(save_p)) > 0; save_p = NEXT_BLKP(save_p)) {
    if (!GET_ALLOC(HDRP(save_p)) && (asize <= GET_SIZE(HDRP(save_p)))) {
      return save_p;
    }
  }
  
  for (save_p = heap_listp; save_p < bp; save_p = NEXT_BLKP(save_p)) {
    if (!GET_ALLOC(HDRP(save_p)) && (asize <= GET_SIZE(HDRP(save_p)))) {
      return save_p;
    }
  }
