...
void sigchld_handler(int sig) 
{
  ...
  while(has_next) {
    ...
    has_next = child_pid > 0;
    
    if (WIFSTOPPED(status)) {
      int child_jid = pid2jid(child_pid);
      ...
      int child_jid = pid2jid(child_pid);
      int success = deletejob(jobs, child_pid);
      if (success) {
...
void sigint_handler(int sig) 
{
  pid_t pid = fgpid(jobs); // retrieve PID of fg job
  
  if (pid > 0) { // if there is a fg job
    // fprintf(stdout, "Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, sig);
    kill(-pid, SIGINT); // send SIGINT signal to it
  }
