...
applyFilter(class Filter *filter, cs1300bmp *input, cs1300bmp *output)
{
  ...
  // Rearrange calls to be outside of the loops so it's not called every iteration.
  ...
  /*
  for(short i=0; i<3; i++) {
    f[i][0] = filter->get(i,0);
    f[i][1] = filter->get(i,1);
    f[i][2] = filter->get(i,2);
  }
  */
  ...
  for(short plane = 0; plane < 3; plane++) {
    for(short row = 1; row < height; row++) {
      for(short col = 1; col < width; col++) {
        ...
        // output -> color[plane][row][col] = 0; (Replaced with temp=0)
        ...
        */
        // Unrolling interior loop
        
        short temp = 0;
        
        temp += input -> color[plane][row+0-1][col+0-1]*f[0][0];
        temp += input -> color[plane][row+0-1][col+1-1]*f[0][1];
        temp += input -> color[plane][row+0-1][col+2-1]*f[0][2];

        temp += input -> color[plane][row+1-1][col+0-1]*f[1][0];
        temp += input -> color[plane][row+1-1][col+1-1]*f[1][1];
        temp += input -> color[plane][row+1-1][col+2-1]*f[1][2];

        temp += input -> color[plane][row+2-1][col+0-1]*f[2][0];
        temp += input -> color[plane][row+2-1][col+1-1]*f[2][1];
        temp += input -> color[plane][row+2-1][col+2-1]*f[2][2];
